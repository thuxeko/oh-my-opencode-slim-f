  ...
  describe('resolveInstallContext', () => {
    test('detects OpenCode packages install root from runtime package path', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
          '/home/user/.cache/opencode/packages/oh-my-opencode-slim-f@latest/package.json',
      );
      ...
      const context = resolveInstallContext(
        '/home/user/.cache/opencode/packages/oh-my-opencode-slim-f@latest/node_modules/oh-my-opencode-slim-f/package.json',
      );

      expect(context).toEqual({
        installDir:
          '/home/user/.cache/opencode/packages/oh-my-opencode-slim-f@latest',
        packageJsonPath:
          '/home/user/.cache/opencode/packages/oh-my-opencode-slim-f@latest/package.json',
      });
      ...
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          p ===
            '/home/user/.cache/opencode/packages/oh-my-opencode-slim-f@latest/package.json' ||
          p ===
            '/home/user/.cache/opencode/packages/oh-my-opencode-slim-f@latest/node_modules/oh-my-opencode-slim-f',
      );
    ...
    test('keeps working when dependency is already on target version', async () => {
      const existsSpy = spyOn(fs, 'existsSync').mockImplementation(
        (p: string) =>
          ...
          p.endsWith('/.cache/opencode/node_modules/oh-my-opencode-slim-f'),
      );
      const readSpy = spyOn(fs, 'readFileSync').mockReturnValue(
        JSON.stringify({
          dependencies: {
            'oh-my-opencode-slim-f': '1.0.1',
          },
        }),
      );
      const writeSpy = spyOn(fs, 'writeFileSync').mockImplementation(() => {});
      ...
      const result = preparePackageUpdate('1.0.1', 'oh-my-opencode-slim-f', null);
