...
Skills are installed via the `oh-my-opencode-slim-f` installer or manually with `npx skills add`.
...
Control which skills each agent can use in `~/.config/opencode/oh-my-opencode-slim-f.json` (or `.jsonc`):
